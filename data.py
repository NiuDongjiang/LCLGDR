...
class DrugNovoLoader(object):
    def __init__(self,
        ...
        self.drug_graph1, self.drug_graph2, self.disease_graph1, self.disease_graph2 = self._generate_feat_graph()
        self._generate_feat()
    ...
    def _generate_feat_graph(self):
        ...
        drug_neighbor1 = np.argpartition(-drug_sim, kth=int(drug_num_neighbor / 2), axis=1)[:,
                         :int(drug_num_neighbor / 2)]
        drug_neighbor_sort = np.argsort(-drug_sim[np.arange(drug_sim.shape[0])[:, None], drug_neighbor], axis=1)
        drug_neighbor2 = drug_neighbor[
            np.arange(drug_sim.shape[0])[:, None], drug_neighbor_sort[:, int(drug_num_neighbor / 2):drug_num_neighbor]]
        dr_row_index1 = np.arange(drug_neighbor1.shape[0]).repeat(drug_neighbor1.shape[1])
        dr_row_index2 = np.arange(drug_neighbor2.shape[0]).repeat(drug_neighbor2.shape[1])
        dr_col_index1 = drug_neighbor1.reshape(-1)
        dr_col_index2 = drug_neighbor2.reshape(-1)
        drug_edge_index1 = np.array([dr_row_index1, dr_col_index1]).astype(int).T
        drug_edge_index2 = np.array([dr_row_index2, dr_col_index2]).astype(int).T

        drug_edges1 = np.array(list(drug_edge_index1), dtype=np.int32).reshape(drug_edge_index1.shape)
        drug_edges2 = np.array(list(drug_edge_index2), dtype=np.int32).reshape(drug_edge_index2.shape)
        drug_adj1 = sp.coo_matrix((np.ones(drug_edges1.shape[0]), (drug_edges1[:, 0], drug_edges1[:, 1])),
                                  shape=(self.num_drug, self.num_drug),
                                  dtype=np.float32)
        drug_adj2 = sp.coo_matrix((np.ones(drug_edges2.shape[0]), (drug_edges2[:, 0], drug_edges2[:, 1])),
                                  shape=(self.num_drug, self.num_drug),
                                  dtype=np.float32)
        drug_adj1 = drug_adj1 + drug_adj1.T.multiply(drug_adj1.T > drug_adj1) - drug_adj1.multiply(
            drug_adj1.T > drug_adj1)
        drug_adj2 = drug_adj2 + drug_adj2.T.multiply(drug_adj2.T > drug_adj2) - drug_adj2.multiply(
            drug_adj2.T > drug_adj2)
        # drug_graph = normalize(drug_adj + sp.eye(drug_adj.shape[0]))
        drug_graph1 = normalize(drug_adj1)
        drug_graph2 = normalize(drug_adj2)
        drug_graph1 = sparse_mx_to_torch_sparse_tensor(drug_graph1)
        drug_graph2 = sparse_mx_to_torch_sparse_tensor(drug_graph2)
        ...
        disease_neighbor1 = np.argpartition(-disease_sim, kth=int(disease_num_neighbor / 2), axis=1)[:,
                            :int(disease_num_neighbor / 2)]
        disease_neighbor_sort = np.argsort(-disease_sim[np.arange(disease_sim.shape[0])[:, None], disease_neighbor],
                                           axis=1)
        disease_neighbor2 = disease_neighbor[np.arange(disease_sim.shape[0])[:, None], disease_neighbor_sort[:,
                                                                                       int(disease_num_neighbor / 2):disease_num_neighbor]]
        di_row_index1 = np.arange(disease_neighbor1.shape[0]).repeat(disease_neighbor1.shape[1])
        di_row_index2 = np.arange(disease_neighbor2.shape[0]).repeat(disease_neighbor2.shape[1])
        di_col_index1 = disease_neighbor1.reshape(-1)
        di_col_index2 = disease_neighbor2.reshape(-1)
        disease_edge_index1 = np.array([di_row_index1, di_col_index1]).astype(int).T
        disease_edge_index2 = np.array([di_row_index2, di_col_index2]).astype(int).T

        disease_edges1 = np.array(list(disease_edge_index1), dtype=np.int32).reshape(disease_edge_index1.shape)
        disease_edges2 = np.array(list(disease_edge_index2), dtype=np.int32).reshape(disease_edge_index2.shape)
        disease_adj1 = sp.coo_matrix((np.ones(disease_edges1.shape[0]), (disease_edges1[:, 0], disease_edges1[:, 1])),
                                     shape=(self.num_disease, self.num_disease),
                                     dtype=np.float32)
        disease_adj2 = sp.coo_matrix((np.ones(disease_edges2.shape[0]), (disease_edges2[:, 0], disease_edges2[:, 1])),
                                     shape=(self.num_disease, self.num_disease),
                                     dtype=np.float32)
        disease_adj1 = disease_adj1 + disease_adj1.T.multiply(disease_adj1.T > disease_adj1) - disease_adj1.multiply(
            disease_adj1.T > disease_adj1)
        disease_adj2 = disease_adj2 + disease_adj2.T.multiply(disease_adj2.T > disease_adj2) - disease_adj2.multiply(
            disease_adj2.T > disease_adj2)
        # disease_graph = normalize(disease_adj + sp.eye(disease_adj.shape[0]))
        disease_graph1 = normalize(disease_adj1)
        disease_graph2 = normalize(disease_adj2)
        disease_graph1 = sparse_mx_to_torch_sparse_tensor(disease_graph1)
        disease_graph2 = sparse_mx_to_torch_sparse_tensor(disease_graph2)

        return drug_graph1, drug_graph2, disease_graph1, disease_graph2
...
class DrugDataLoader(object):
    ...
    def _generate_feat_graph(self):
        ...
        drug_neighbor = np.argpartition(-drug_sim, kth=drug_num_neighbor, axis=1)[:, :drug_num_neighbor]
        drug_neighbor1 = np.argpartition(-drug_sim, kth=int(drug_num_neighbor/2), axis=1)[:, :int(drug_num_neighbor/2)]
        drug_neighbor_sort = np.argsort(-drug_sim[np.arange(drug_sim.shape[0])[:, None], drug_neighbor], axis=1)
        drug_neighbor2 = drug_neighbor[np.arange(drug_sim.shape[0])[:, None], drug_neighbor_sort[:, int(drug_num_neighbor/2):drug_num_neighbor]]
        dr_row_index1 = np.arange(drug_neighbor1.shape[0]).repeat(drug_neighbor1.shape[1])
        dr_row_index2 = np.arange(drug_neighbor2.shape[0]).repeat(drug_neighbor2.shape[1])
        dr_col_index1 = drug_neighbor1.reshape(-1)
        dr_col_index2 = drug_neighbor2.reshape(-1)
        ...
        disease_neighbor = np.argpartition(-disease_sim, kth=disease_num_neighbor, axis=1)[:, :disease_num_neighbor]
        disease_neighbor1 = np.argpartition(-disease_sim, kth=int(disease_num_neighbor / 2), axis=1)[:, :int(disease_num_neighbor / 2)]
        disease_neighbor_sort = np.argsort(-disease_sim[np.arange(disease_sim.shape[0])[:, None], disease_neighbor], axis=1)
        disease_neighbor2 = disease_neighbor[np.arange(disease_sim.shape[0])[:, None], disease_neighbor_sort[:, int(disease_num_neighbor / 2):disease_num_neighbor]]
        di_row_index1 = np.arange(disease_neighbor1.shape[0]).repeat(disease_neighbor1.shape[1])
        di_row_index2 = np.arange(disease_neighbor2.shape[0]).repeat(disease_neighbor2.shape[1])
        di_col_index1 = disease_neighbor1.reshape(-1)
        di_col_index2 = disease_neighbor2.reshape(-1)
